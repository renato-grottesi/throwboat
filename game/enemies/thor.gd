...
onready var animation = get_node("thor")
...
func _ready():
	...
	dying_tween.interpolate_property(animation, "visibility/opacity", 1.0, 0.0, 1.0, Tween.TRANS_SINE, Tween.EASE_IN)
...
func face_ship():
	if shoot_target == target_e.PLAYER:
		...
		if(enemy!=null):
			set_rot((get_pos()-enemy.get_pos()).angle() + initial_rotation)
...
func gets_hurt(damage, pos, normal, emitter, travel_time):
	...
	if health <= 0:
		...
		dying_tween.interpolate_property(animation, "transform/rot", get_rot(), get_rot()+32.0*PI, 1.0, Tween.TRANS_LINEAR, Tween.EASE_IN)
		dying_tween.start()
...
func _on_sprite_finished():
	if status == status_e.FLYING:
		...
		if shoot_pattern == pattern_e.SINGLE:
			emit_signal("emit_bullet", "hammer", self, get_node("origin").get_global_pos(), get_rot(), bullet_speed, bullet_bounces)
			get_node("sounds").play("shoot")
			shoots+=1
		if shoot_pattern == pattern_e.SPREAD:
			var angle = get_rot()
			var vec = get_node("origin").get_pos()
			emit_signal("emit_bullet", "hammer", self, vec.rotated(angle) + get_global_pos(), angle, bullet_speed, bullet_bounces)
			for b in range(1, 1+shoot_amount/2):
				var sub_angle = deg2rad(shoot_degree_step*b)
				emit_signal("emit_bullet", "hammer", self, vec.rotated(angle + sub_angle) + get_global_pos(), angle + sub_angle, bullet_speed, bullet_bounces)
				emit_signal("emit_bullet", "hammer", self, vec.rotated(angle - sub_angle) + get_global_pos(), angle - sub_angle, bullet_speed, bullet_bounces)
			get_node("sounds").play("shoot")
			shoots+=shoot_amount
		if shoot_pattern == pattern_e.ROTATION:
