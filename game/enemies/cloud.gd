...
func _on_sprite_finished():
	if status == status_e.FLYING:
		...
		if shoot_pattern == pattern_e.SINGLE:
			emit_signal("emit_bullet", "lightning", self, get_node("origin").get_global_pos(), get_rot(), bullet_speed, bullet_bounces)
			get_node("sounds").play("shoot")
			shoots+=1
		if shoot_pattern == pattern_e.SPREAD:
			var angle = get_rot()
			var vec = get_node("origin").get_pos()
			emit_signal("emit_bullet", "lightning", self, vec.rotated(angle) + get_global_pos(), angle, bullet_speed, bullet_bounces)
			for b in range(1, 1+shoot_amount/2):
				var sub_angle = deg2rad(shoot_degree_step*b)
				emit_signal("emit_bullet", "lightning", self, vec.rotated(angle + sub_angle) + get_global_pos(), angle + sub_angle, bullet_speed, bullet_bounces)
				emit_signal("emit_bullet", "lightning", self, vec.rotated(angle - sub_angle) + get_global_pos(), angle - sub_angle, bullet_speed, bullet_bounces)
			get_node("sounds").play("shoot")
			shoots+=shoot_amount
		if shoot_pattern == pattern_e.ROTATION:
